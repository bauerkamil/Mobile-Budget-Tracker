...
export const getUserRecurringExpenses = async (): Promise<IRecurringExpense[] | undefined> => {
  try {
...
export const removeRecurringExpense = async (recurringExpenseId: string) => {
  try {
    const recurringExpenseRef = doc(firestore, RECURRING_EXPENSES_TABLE_NAME, recurringExpenseId);
